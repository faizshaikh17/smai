...
const CodeBlock = ({ code, language }: { code: string; language: string }) => (
  <div className="p-1 mx-auto border border-neutral-600/50 rounded-[1.1rem] bg-neutral-900 shadow-[inset_0_4px_6px_#242629,inset_0_1px_2px_rgba(255,255,255,0.04),inset_0_3px_2px_rgba(209,213,219,0.1),0_3px_10px_rgba(0,0,0,0.25),0_1px_1px_rgba(255,255,255,0.06)] w-full overflow-hidden">
    <div className="p-4 sm:p-6 md:p-8 text-[clamp(0.75rem,1vw,1rem)] leading-[1.5] bg-[var(--background)] rounded-xl shadow-[0_2px_6px_rgba(0,0,0,0.2),0_-1px_2px_rgba(209,213,219,0.05),inset_0_1px_0_rgba(255,255,255,0.01),0_-2px_3px_rgba(0,0,0,0.12)]">
      <SyntaxHighlighter
        language={language}
        style={tomorrow}
        wrapLongLines
...
const CopyButton = ({ text }: { text: string }) => {
  ...
  return (
    <button
      type="button"
      onClick={handleCopy}
      className="flex absolute text-neutral-400 md:top-6 top-3 right-2 md:right-4 items-center gap-2 px-4 rounded"
    >
...
const data = await response.json();`,
  ...
  return (
    <section className="relative py-[12rem] w-full flex flex-col items-center overflow-hidden">
      <div className="flex flex-col w-full z-50 items-center mx-auto relative px-4 sm:px-6">
        ...
        <div className="bg-gradient-to-b w-full p-2 sm:p-4 rounded-[2.5rem] md:rounded from-teal-100 via-blue-500 to-transparent">
...
const client = new OpenAI({
  ...
  https://api.openai.com/v1/"})`}
      ...
      <section className="relative pt-[12rem] w-full flex flex-col items-center overflow-hidden px-4 sm:px-6">
        <div className="flex flex-col w-full gap-12 items-center relative z-10">
          <div className="mb-12 flex flex-col items-center">
            <div className="text-xs uppercase text-gray-400 mb-4">
              FEATURES • FEATURES • FEATURES
            </div>
            <h2 className="text-[clamp(1.5rem,3vw,2.5rem)] font-bold leading-[1.2] text-white text-center max-w-[50rem]">
          ...
          <div className="flex w-full max-w-5xl flex-col">
            <div className="flex bg-white flex-col rounded-xl py-2">
              <div className="flex flex-wrap py-2 px-4 sm:px-6 border-b border-neutral-600/30">
                <div className="bg-neutral-300 p-1 relative flex flex-wrap rounded-2xl overflow-hidden w-full md:w-auto">
                  {tabs.map((tab) => (
                    <button
                      key={tab.name}
                      type="button"
                      onClick={() => setActiveTab(tab.name)}
                      className={`flex-grow md:flex-grow-0 md:px-3 md:py-3 py-2 md:leading-none md:font-normal leading-[100%] text-sm font-semibold md:text-base rounded-xl ${
                        activeTab === tab.name
